...
In addition, your movement may affect the state of the ship.  If you open a hatch between two rooms, and one room is flooded to high water, it will flow into the other room as long as there is no water already there.  The attached room will flood to Low Flood levels.   The original room that was flooded to high water will also reduce its water level to Low Flood, which you can then walk through -- for an additional minute of your time.  By thoughtfully flooding rooms, you may be able to create a more direct path to your target room or even put out a fire along the way.
...
The custom show instance increases readability during debugging by adding newlines between the lists, and can be pasted directly back into the interpreter if needed.
...
The Actions list may include a variety of action types, but only the StartRoom and Move actions imply that a Room was visited.  The getRoom function gets the Just Room from those relevant actions, and Nothing from the rest. 
...
And, to determine whether some room can be the target of our next Move, we have a simple isVisited function to describe whether a room has been visited or not.
...
The order of actions in the GameState is important.  The starting room is the last element of the Actions list, and the final (or current) room is the head of the actions list.  When we add a new action to the list, we will add it to the head.
...
There are two GameStates returned in a list.  The first one shows the movement to Room 2 with a cost of 1, due to the LowFlood, and the second shows a movement to Room 3. As we expected, the moveRooms function does not allow a movement into Room 4.
